...
from model_generator import S3_BUCKET, create_aml_model_with_csv
...
if args.upload_cleaned_data:
    print("Uploading file cleaned_combined.csv to S3 into bucket ", S3_BUCKET)
    s3 = boto3.resource('s3')
    bucket = s3.Bucket(S3_BUCKET)
    bucket.upload_file("csv_files/cleaned_combined.csv", "cleaned_combined.csv")

if args.create_aml_model:
    create_aml_model_with_csv(
        included_columns=['truckMiles', 'price_per_mile', 'numVehicles'],
                          # 'rv', 'van', 'travel trailer', 'car', 'atv', 'suv', 'pickup',
                          # 'motorcycle', 'heavy equipment', 'boat'],
        csv_file_name='cleaned_combined.csv',
